...
describe('todos:saveSnapshot / todos:listSnapshots（待办回合快照持久化）', () => {
  let userDataDir: string
  let snapshotFile: (sessionId: string) => string
  const makeSnap = (round: number, extra?: Partial<{ endedAt: number; todos: unknown[] }>) => ({
    round,
    endedAt: extra?.endedAt ?? 1_700_000_000_000 + round,
    todos: extra?.todos ?? [{ content: `任务${round}`, status: 'completed' }],
    completedAll: true,
  })

  beforeEach(async () => {
    userDataDir = await fsp.mkdtemp(join(tmpdir(), 'ipc-todo-'))
    snapshotFile = (sessionId: string) => join(userDataDir, 'data', 'session-todos', `${sessionId}.json`)
    electronMock.app.getPath.mockReset()
    electronMock.app.getPath.mockImplementation((name: string) => (name === 'userData' ? userDataDir : ''))
  })
  ...
  it('saveSnapshot 写入文件，listSnapshots 读回（含 round/todos/completedAll）', async () => {
    registerIpcHandlers()
    const save = electronMock.handleCalls.find((x) => x.channel === 'todos:saveSnapshot')!
    const list = electronMock.handleCalls.find((x) => x.channel === 'todos:listSnapshots')!
    const r = (await save.handler({}, { sessionId: 'ses_a', maxSnapshots: 20, snapshot: makeSnap(1) })) as { ok: boolean }
    expect(r).toEqual({ ok: true })
    const back = (await list.handler({}, { sessionId: 'ses_a' })) as { snapshots: Array<Record<string, unknown>> }
    expect(back.snapshots).toHaveLength(1)
    expect(back.snapshots[0]!.round).toBe(1)
    expect(back.snapshots[0]!.todos).toEqual([{ content: '任务1', status: 'completed' }])
  })

  it('多次追加 + 超 maxSnapshots 删最旧（保留最近 N 轮）', async () => {
    registerIpcHandlers()
    const save = electronMock.handleCalls.find((x) => x.channel === 'todos:saveSnapshot')!
    // 连续保存 3 轮，maxSnapshots=2 → 只保留第 2、3 轮
    for (const round of [1, 2, 3]) {
      const r = (await save.handler({}, { sessionId: 'ses_b', maxSnapshots: 2, snapshot: makeSnap(round) })) as { ok: boolean }
      expect(r.ok).toBe(true)
    }
    const back = (await (electronMock.handleCalls.find((x) => x.channel === 'todos:listSnapshots')!).handler({}, { sessionId: 'ses_b' })) as { snapshots: Array<{ round: number }> }
    expect(back.snapshots.map((s) => s.round)).toEqual([2, 3])
  })

  it('参数校验：sessionId 非法 / todos 超 50 / endedAt 非有限 → { ok:false, error }', async () => {
    registerIpcHandlers()
    const save = electronMock.handleCalls.find((x) => x.channel === 'todos:saveSnapshot')!
    expect((await save.handler({}, { sessionId: 'bad', maxSnapshots: 20, snapshot: makeSnap(1) }))).toEqual({ ok: false, error: 'sessionId 非法' })
    const tooMany = Array.from({ length: 51 }, (_, i) => ({ content: `t${i}`, status: 'pending' }))
    expect((await save.handler({}, { sessionId: 'ses_c', maxSnapshots: 20, snapshot: makeSnap(1, { todos: tooMany }) }))).toEqual({ ok: false, error: '快照参数非法' })
    expect((await save.handler({}, { sessionId: 'ses_c', maxSnapshots: 20, snapshot: makeSnap(1, { endedAt: NaN }) }))).toEqual({ ok: false, error: '快照参数非法' })
  })

  it('maxSnapshots 非法（<1 或 >100）→ 回退默认 20（不报错）', async () => {
    registerIpcHandlers()
    const save = electronMock.handleCalls.find((x) => x.channel === 'todos:saveSnapshot')!
    // maxSnapshots=0（非法）→ 默认 20：写 3 轮仍全保留
    for (const round of [1, 2, 3]) {
      const r = (await save.handler({}, { sessionId: 'ses_d', maxSnapshots: 0, snapshot: makeSnap(round) })) as { ok: boolean }
      expect(r.ok).toBe(true)
    }
    const back = (await (electronMock.handleCalls.find((x) => x.channel === 'todos:listSnapshots')!).handler({}, { sessionId: 'ses_d' })) as { snapshots: unknown[] }
    expect(back.snapshots).toHaveLength(3)
  })

  it('损坏文件忽略：旧文件 JSON 坏 → 从零重建追加（不抛错）', async () => {
    const file = snapshotFile('ses_e')
    await fsp.mkdir(join(file, '..'), { recursive: true })
    await fsp.writeFile(file, '{ 坏 JSON', 'utf-8')
    registerIpcHandlers()
    const save = electronMock.handleCalls.find((x) => x.channel === 'todos:saveSnapshot')!
    const r = (await save.handler({}, { sessionId: 'ses_e', maxSnapshots: 20, snapshot: makeSnap(1) })) as { ok: boolean }
    expect(r).toEqual({ ok: true })
    const back = (await (electronMock.handleCalls.find((x) => x.channel === 'todos:listSnapshots')!).handler({}, { sessionId: 'ses_e' })) as { snapshots: unknown[] }
    expect(back.snapshots).toHaveLength(1)
  })

  it('listSnapshots：文件不存在 / 损坏 / sessionId 非法 → 空数组（不抛错）', async () => {
    registerIpcHandlers()
    const list = electronMock.handleCalls.find((x) => x.channel === 'todos:listSnapshots')!
    expect(await list.handler({}, { sessionId: 'ses_missing' })).toEqual({ snapshots: [] })
    // 损坏文件
    const file = snapshotFile('ses_bad')
    await fsp.mkdir(join(file, '..'), { recursive: true })
    await fsp.writeFile(file, '不是 JSON', 'utf-8')
    expect(await list.handler({}, { sessionId: 'ses_bad' })).toEqual({ snapshots: [] })
    expect(await list.handler({}, { sessionId: '' })).toEqual({ snapshots: [] })
  })
})
