...
| v1.1 | 2026-08-09 | 实测修订：①列表全量 v2（v1 `/session` limit 参数 1.18.15 起失效）→ serveDirs 聚合不再只聚合最近 50 条；②同工作区窗口去重（winWorkspaces + `window:registerWorkspace` 主窗口补洞——主窗口创建时 workspace 未传登记为空，切走再切回会新开窗口）；③会话跟随工作区改前端 normalizeDir 过滤（serve 带 directory 实例化曾崩溃） | 技术研发 |
