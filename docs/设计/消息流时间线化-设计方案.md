...
| D10 收起态梗概 | 按节点类型提取：思考=内容前 60 字 / read=文件路径 / edit=文件路径 / bash=命令首行 / websearch=查询词 / task=子智能体任务 / compress=压缩摘要 / todowrite=当前进行中任务 | 用户确认（demo v3） |
| D11 todo 节点 | todowrite = **单行**（`📋 更新待办 · 正在：<进行中任务>` + ✓）——**无展开交互**（独立 TodoPanel 已负责详情） | 用户确认（demo v3） |
...
| todo | 📋 更新待办 · 正在：\<in_progress 任务\> + ✓ | **无展开**（D11） | 常显 |
