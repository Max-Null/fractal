...
flowchart TD
    ...
    G -->|create 临时会话| D
    G -->|buildPolishPrompt<br>text + 引用上下文| D
    G -->|轮询 messages 500ms×120| D
    G -->|extractAssistantText| F
    F -->|回填 input| A
    G -->|delete 临时会话| D
```
...
| D6 polish 走引擎 | 临时会话 → promptAsync → 轮询 messages → extractAssistantText → 删会话 | OC 唯一引擎架构铁律：不引入独立 LLM 直连通道；临时会话隔离，不污染当前会话历史（评论/追问历史不可预期、贵、慢） |
| D7 polish 参数固定 | agent: `build` + model: `deepseek-v4-flash` + variant: `low` | build agent 纯文本不执行工具（默认双星会读文件/多轮工具流——用户报错根因）；flash+low 推理快 2-3 倍，润色是短文本任务；60s 超时兜底长推理（20s 曾触发「润色超时」） |
...
**文件：** `electron/main/ipc.ts`（L639-671）

```typescript
// ✨ 输入消息优化：临时会话润色 → 提取回复 → 删除临时会话。
...
ipcMain.handle('ai:polishMessage', async (_e, args: { text: string; refs?: Array<{ label?: string; content?: string; path?: string }> }) => {
  ...
  try {
    ...
    // agent: build——纯文本润色不执行工具；model: flash + variant low——短文本任务快模型足够
...
2. **临时会话**：`session.create({ title: '消息润色' })`，不带 cwd 绑定（复用 serve 默认工作区）；临时会话隔离，润色历史不进入用户当前会话。
3. **promptAsync 异步提交**：返回 204 后结果走 serve 内部生成，主进程无法直接拿到——因此轮询 `session.messages(tempId)`（500ms 间隔 × 120 次 = 60s 上限），直到 `extractAssistantText` 提取到非空 assistant 文本。
4. **超时兜底**：60s 内无回复 → throw「润色超时：模型未在 60 秒内回复」（deepseek-v4-pro 长推理曾触发 20s 超时，故放宽）。
5. **清理**：`finally` 中删除临时会话；删除失败不阻断（serve 残留一条「消息润色」会话可接受）。
...
### 3.3 extractAssistantText — 润色回复提取

**文件：** `electron/main/ipc.ts`（L1035-1046）
...
- **失败分支**：主进程返回 `ok: false`（理论不达，防御）→ 「优化失败：引擎未返回结果」；主进程 throw → invoke reject → 提取真实 message（去 `Error invoking remote method 'xxx':` 前缀）展示具体原因。
...
| 操作 | 文件 | 说明 |
|------|------|------|
| 修改 | `electron/main/ipc.ts` | 新增 `ai:polishMessage` handler + `POLISH_PROMPT` / `POLISH_REF_MAX_BYTES` / `buildPolishPrompt` / `extractAssistantText`；`chat:sendMessage` 透传 agent/variant；`buildSendParts` / `mimeFromExt`（附件发送复用） |
| 修改 | `electron/main/oc-sdk.ts` | `promptPartsAsync`（FilePartInput 透传 + variant 变量透传） |

### 渲染进程

| 操作 | 文件 | 说明 |
|------|------|------|
| 修改 | `src/renderer/src/components/chat/InputBar.vue` | composer 单卡片三层重构 + chips 行 + 操作行 + 输入行 + 渐变提示 + polish 按钮/错误条 + 斜杠菜单定位修正 |
| 修改 | `src/renderer/src/components/chat/ChatPanel.vue` | `composerChips` 组装（textSnippet + attachedFiles）；`handleSend` agent/variant 透传；InputBar 挂载 + left slot debug 按钮 |
...
| 修改 | `src/renderer/src/locales/zh.json` / `en.json` | `composer.*` 命名空间（agent/model/perm/effort/polish 等文案） |

### 测试

| 操作 | 文件 | 说明 |
|------|------|------|
| 修改 | `src/renderer/src/components/chat/InputBar.test.ts` | 25 用例：发送/Enter/Stop/空输入/拖放/chips 空态/agent/model pill/polish ×3/attach/effort 选择器 ×4 |
| 修改 | `electron/main/ipc.test.ts` | `extractAssistantText` 3 用例 + `buildPolishPrompt` 4 用例 |
| 修改 | `src/renderer/src/components/chat/ChatPanel.test.ts` | P6 附件发送 → sendMessage attachments |
...
sequenceDiagram
    ...
    IPC->>SDK: session.create({title:'消息润色'})
    SDK-->>IPC: tempId
    IPC->>IPC: buildPolishPrompt(text, refs)（读附件文件 50KB 截断）
    IPC->>SDK: promptAsync(tempId, prompt, {agent:'build', model:flash, variant:'low'})
    SDK-->>IPC: 204 accepted
    loop 轮询 ≤ 120 次（500ms 间隔 = 60s 上限）
        IPC->>SDK: session.messages(tempId)
        SDK-->>IPC: 消息列表
        IPC->>IPC: extractAssistantText → 非空即跳出
    end
    IPC-->>B: { ok: true, text: 润色结果 }
    B-->>IB: result
    IB-->>用户: 输入框回填润色结果 + autoResize
    IPC->>SDK: session.delete(tempId)（finally 清理）
```
...
| 润色 60s 无回复 | 错误条显示「润色超时：模型未在 60 秒内回复」，3s 消失 | 轮询 120 次后 throw |
| 引用文件读失败（已删除/无权限） | 无感知（润色照常） | `buildPolishPrompt` catch 跳过该引用，不阻断 |
| 临时会话创建失败 | 错误条显示原因 | throw「临时会话创建失败」 |
| 临时会话删除失败 | 无感知 | catch 忽略（serve 残留「消息润色」会话可接受） |
| 主进程返回 ok=false | 错误条「优化失败：引擎未返回结果」 | —（理论不达，防御） |
| 润色进行中重复点击 | 按钮 busy 禁用 | —（`polishing` 标志防重入） |
| 引擎未初始化 | invoke reject → 错误条显示具体原因 | requireClient 抛错 |
...
| 润色超时 | 主进程 mock 超时 | 点 ✨ | 错误条「润色超时：模型未在 60 秒内回复」 |
...
| extractAssistantText | mock 消息列表 | 调用 | 取最后一条 assistant 多 text part 拼接；无文本跳过 |
...
| 临时会话残留 | 低 | finally 尽力删除；删除失败 serve 侧残留「消息润色」会话，可接受 |
| 润色结果污染输入框（模型输出多余解释） | 中 | prompt 明确约束「直接输出优化后的消息，不要任何解释、前缀或引号」 |
| 润色超时 | 低 | 60s 轮询上限 + 明确错误提示（flash+low 组合已提速） |
