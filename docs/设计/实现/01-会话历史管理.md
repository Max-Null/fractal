...
| 变更 | 原因 |
|------|------|
| **设计基线精确化（v1.2 用户澄清）** | 「serve 唯一数据源、无本地库」精确表述为：**OC 能提供的数据** → serve 唯一源不存本地；**OC 无法提供的数据** → 分形做**辅助处理**（本地存储，如记忆机制 memories/blocks）。todo 每轮快照（serve 只有最新状态）即属辅助处理范畴 |
...
| # | 缺口 | 决策 | 实施要点 |
|---|------|------|---------|
| G1 | 无本地缓存层 | **A. 保持 serve 唯一源**（不做 SQLite 镜像）——**2026-08-09 用户澄清精确化**：OC 能提供的数据 serve 唯一源；**OC 无法提供的（todo 每轮快照等）→ 分形辅助处理本地存储**（记忆机制同款原则） | ✅ 已定稿：排序 = serve `time.created` 倒序（2026-08-08 探针实测，前端依赖 serve 顺序）；重命名 ✅ serve PATCH 实测通过；归档 = experimental API 雏形，不可依赖 |
