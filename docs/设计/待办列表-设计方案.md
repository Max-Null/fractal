...
| D8 快照内容 | `{ snapshots: [{ round, endedAt, todos[], completedAll }] }`，每会话上限 20 条，单快照 todo ≤ 50 条截断 | 小 JSON 按会话一文件，防膨胀 |
...
```
入参 { sessionId: string; snapshot: { round: number; endedAt: number; todos: Array<{content, status, priority}>; completedAll: boolean } }
行为：
...
| 修改 | `src/renderer/src/stores/chat.ts` | +todoSnapshots/load/push/自动展开联动/回合收尾快照 |
...
| 修改 | `src/renderer/src/locales/zh.json` / `en.json` | 记录卡/折叠态文案 |
| 修改 | 测试：ipc.test / chat.test / TodoPanel.test / ChatPanel.test / useStreamProcessor.test | 新增用例 |
