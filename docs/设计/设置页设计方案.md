...
| 编号 | 决策 | 理由 | 备注 |
|---|---|---|---|
| **D1 数据模式开关** | 新增「独立会话数据」开关（默认**关**=共享） | 用户需求：数据共享/独立由用户决定；默认共享延续现状（无缝接管已有会话） | 核心新增，见四 |
...
| 状态 | 注入 | 会话来源 | 适用 |
|---|---|---|---|
| 关（默认） | 不注入 XDG_DATA_HOME | `~/.local/share/opencode`（全局共享） | 默认：无缝接管官方桌面端/CLI 会话 |
| 开 | `XDG_DATA_HOME=<userData>/data` | `<userData>/data/opencode`（仅本应用） | 完全隔离：分形只见自己的会话 |
