...
| 引擎 | OC 唯一（`opencode serve` + SDK，SSE 事件流），无 CC 代码 |
...
### 10.1.1 实施状态（2026-08-10 核对，git log + 改造记录）

| 阶段 | 状态 | 提交证据 |
|------|------|---------|
| 阶段 0 | ✅ 完成 | 阶段0-实测记录.md + 附录 C 回填 |
| 阶段 1-2 | ✅ 完成 | fe0e038 |
| 阶段 3-5 | ✅ 完成 | 5070b93 + 92840f7（question/权限/todo 真实接入） |
| 阶段 6 配置体系 | ✅ 完成 | b0323e5（settings.json + schema + JSONC 编辑器 + 文件监听 + Agent 协作） |
| 阶段 7 sidecar | ✅ 完成 | ed6773e（内嵌 OC 二进制 + 独立运行） |
| 阶段 8 预置体系 | ✅ 完成 | 32c6682（oc-plus 全家桶 + 首次初始化）；后续演进：agents-manifest.json 契约驱动模型槽位（5330e0f）、预置 mtime 保护（a4d4b2d） |
| 阶段 9 收尾 | ✅ 完成 | c5111a8（实现文档 03-07 + release notes + 模块地图） |

**二期新增功能（一期后，2026-08-07~08-10，均已完成）**：消息流时间线化（3a/3b + 8 项反馈）、子智能体可视化（三态卡片 + 监视/详情弹窗 + 历史子任务平铺）、待办列表 v2（纯 serve 数据源）、模型槽位统一（HIGH/LOW/VISION + kimi 多模态）、工作区管理（多窗口 + 最近使用）、载入页 Si 原子（Canvas 3D）、诊断面板日志落盘、serve 稳定性系列（1.18.15 + 并发互斥 + 串行门禁 + 崩溃 dump）、历史消息分页、时间线完整目录 + 内存分页渲染、8800 端口冲突认知更正（V2 指示器体系清理）。

> 各功能设计与实测结论见 `docs/设计/` 独立方案文档（消息流时间线化 v1.4、子智能体可视化 v1.1、待办列表 v2.0、模型配置 v1.2 等）；待定问题清单 P4-P8 已全部回填关闭（2026-08-10）。
...
| v1.17 | 2026-08-10 | **一期全部完成 + 二期状态回填**（10.1.1 实施状态核对）：阶段 6 配置体系（b0323e5）、阶段 7 sidecar（ed6773e）、阶段 8 预置（32c6682 + agents-manifest 契约驱动 5330e0f + mtime 保护 a4d4b2d）、阶段 9 收尾（c5111a8）均已完成；二期功能（时间线/子智能体可视化/待办 v2/模型槽位/工作区/载入页/诊断面板/serve 稳定性/8800 认知更正）全部落地，详见 10.1.1；待定问题清单 P4-P8 全部回填关闭；子智能体可视化等独立方案文档为最新权威（主方案实施计划不再逐项更新） | 技术研发 |
