...
| 角色 | 可操作范围 | 本次是否覆盖 |
|------|-----------|-------------|
| 新项目脚手架 | Tauri 2 + Vue 3 + TS 工程初始化 | ✅ |
| 前端设计资产迁移 | 布局/会话/文件/工具栏/共享组件/状态层 | ✅ |
| 后端 Rust | ACP 集成（进程/协议/配置）、SQLite schema 迁移 | ✅ |
| 打包 | 可安装包（NSIS/MSI） | ✅ |
...
flowchart TD
    ...
    subgraph RUST["oc-gui Rust 后端（新建）"]
        CMDS["lib.rs — Tauri 命令"]
        PROC["process.rs — 每会话 ACP 进程管理"]
        ACPC["acp_client.rs — JSON-RPC 2.0 客户端"]
        OC_PROTO["oc_protocol.rs — 事件映射层"]
        OCCFG["oc_config.rs — opencode.json 生成"]
    end

    subgraph ENGINE["OC 引擎"]
        CLI["opencode acp 子进程（每会话一个）"]
        SIDECAR["内置 OC sidecar 二进制（externalBin 打包）"]
    end

    LLM["LLM API"]

    UI <-->|"invoke / stream-event（契约不变）"| BRIDGE
    BRIDGE <-->|Tauri IPC| CMDS
    CMDS --> PROC
    PROC --> ACPC
    ACPC <-->|"stdin/stdout NDJSON (JSON-RPC 2.0)"| CLI
    CLI -->|API| LLM
    OC_PROTO -->|"sessionUpdate → StreamFrontendEvent"| CMDS
    OCCFG -.->|"读写 opencode.json"| CLI
...
| D2 集成接口 | `opencode acp`（ACP 双工协议） | 开放标准（Anthropic 提出），`session/request_permission` 原生支持审批，与 cc-gui 双工模型一一对应 |
| D3 前端资产复用 | **整体迁移 + StreamFrontendEvent 作为前后端契约** | 设计资产全部引擎无关，契约不变则组件/状态层/渲染零改造；这是「不重新设计」的落地方式 |
| D4 进程模型 | 每会话一个 ACP 长驻进程，生命周期 = 会话生命周期 | 复用 cc-gui 的 ProcessManager 设计；cwd 隔离，互不干扰 |
...
| D8 独立运行 | **一期内置 OC sidecar**：Tauri `externalBin` 打包官方发布的独立二进制，优先启动内置引擎，系统已装 OC 作为兜底 | 可行性已验证：① Tauri 2 官方支持外部二进制打包（命名带目标三元组后缀，`Command::new_sidecar` 启动）；② OC 官方 GitHub Releases 发布全平台独立二进制（`opencode-windows-x64.zip` 等），无需自行编译 |
| D9 技术选型 | **保留 Tauri 2 + Vue 3 + TypeScript**（与 cc-gui 一致） | 设计资产复用是硬约束——前端组件/状态层/样式全部是 Vue 3 生态，换技术栈等于重新设计；Tauri 2 提供 sidecar 打包与轻量桌面壳。用户授权可改选型，但当前选型是「复用成本最低 + 开箱即用目标」的最优解 |
...
| D12 打包交付 | Tauri bundle（NSIS/MSI），沿用 cc-gui 打包经验 | 独立安装工具的目标形态 |
...
| D16 模型固定 | **deepseek-v4-flash（默认）+ deepseek-v4-pro（预览版标记）高低搭配**，DeepSeek 出新模型前不换 | 用户确认：目前全用 flash（正式版，能力比预览版 pro 强）；pro 正式上市后切默认；映射表维护在 provider.rs |
...
```
src-tauri/src/
├── acp_client.rs     # ACP JSON-RPC 2.0 客户端（连接管理/请求/事件分发）
├── oc_protocol.rs    # OC sessionUpdate 事件 → StreamFrontendEvent 映射层
└── oc_config.rs      # opencode.json 生成（Provider/权限/模型配置）
```

#### 3.1.1 acp_client.rs — ACP 客户端

**职责**：管理 `opencode acp` 子进程的双工通信。stdin 写 JSON-RPC 请求（每行一个 JSON），stdout 读响应与通知（每行一个 JSON）。

**请求方法（Client → Agent）**：

| 方法 | 参数 | 用途 |
|------|------|------|
| `initialize` | 协议版本、能力协商 | 握手，确认 `agentCapabilities`（resume/fork/mcp 支持） |
| `session/new` | `cwd`、`mcpServers`（可选）、`additionalWorkspaceRoots`（附件目录） | 创建会话，返回 `sessionId` |
| `session/prompt` | `sessionId`、`ContentBlock[]` | 发送用户消息；接受后返回，进度经 `session/update` 推送 |
| `session/cancel` | `sessionId` | 取消当前回合 |
| `session/resume` | `sessionId` | 恢复已有会话 |
| `session/close` | `sessionId` | 关闭会话（删除会话时调用） |
| `request_permission` 响应 | `RequestPermissionOutcome` | 返回审批决策 |

**接收通知**：`session/update` 逐行解析后交给 oc_protocol.rs。帧协议复用 cc-gui 的「按行解析 JSON」模式（`StreamLine::parse` 思路）。

#### 3.1.2 oc_protocol.rs — 事件映射层

**职责**：OC sessionUpdate → 现有 `StreamFrontendEvent`（与 cc-gui 契约完全一致）。

| OC sessionUpdate 类型 | StreamFrontendEvent | 说明 |
|------------------------|---------------------|------|
...
| `session/request_permission` | `control_request` | 前端复用现有审批弹窗 |
| `session.status` idle（stop reason + usage） | `result`（is_final） | 回合结束 |
| JSON-RPC error / 进程退出 | `stream-error` / `process-exited` | 错误通道 |

#### 3.1.3 oc_config.rs — 配置文件生成

**职责**：生成/合并 `~/.config/opencode/opencode.json`，并将应用配置中的引擎相关项（DeepSeek API Key 引用、模型、权限模式）同步到 OC 配置。

- DeepSeek 为 OC 内置 provider：仅注入 `options.apiKey`（`{env:OC_GUI_DEEPSEEK_API_KEY}` 引用，运行时由应用注入环境变量）
- 模型列表来自 models.dev 目录；GUI 展示名 ↔ 模型 ID 映射表维护在 provider.rs
- 默认 `model: "deepseek/<model>"`

### 3.2 process.rs — ACP 进程管理

```
优先：Command::new_sidecar("binaries/opencode")   # 内置 sidecar（独立运行）
兜底：spawn 系统安装的 opencode                    # find_opencode()
```

- 启动：会话首次发消息时 spawn，**回合结束后进程保持**
- 退出：会话删除（先 `session/close` 再 kill）、应用退出
- `find_opencode()`：先检测 sidecar 可用性，再检测 npm 全局安装的 `opencode` / `opencode.exe`
- 意外退出：沿用 `process-exited` 通道 + 会话状态置 error
...
3. 重命名加目标三元组后缀：
   Windows → src-tauri/binaries/opencode-x86_64-pc-windows-msvc.exe
   macOS   → src-tauri/binaries/opencode-aarch64-apple-darwin
   Linux   → src-tauri/binaries/opencode-x86_64-unknown-linux-gnu
4. 提交或构建时拉取
```
...
// capabilities/default.json 追加权限
{
  "permissions": [
    "core:default",
    {
      ...
      "allow": [
        { "name": "binaries/opencode", "sidecar": true }
      ]
    }
  ]
}
...
```rust
// 通过 tauri_plugin_shell 启动 sidecar，stdin/stdout 管道与普通 Command 一致
let cmd = app.shell().sidecar("binaries/opencode")
    .map_err(|e| format!("Sidecar 初始化失败: {}", e))?
    .args(["acp", "--cwd", &params.cwd]);
// 后续 stdin/stdout 捕获逻辑与系统 OC 完全相同
```

**风险控制**：
- 版本固定：构建脚本锁死 OC 版本号，避免 sidecar 与协议适配层不匹配
- 更新机制：二进制随应用版本发布（升级应用即升级引擎）；独立热更新留待二期评估
- macOS 签名：sidecar 需代码签名否则 Gatekeeper 拦截（用户当前为 Windows 平台，影响低，但需在发布流程中处理）

### 3.4 lib.rs — Tauri 命令

| 命令 | 设计 |
|------|------|
| `send_message` | 进程存在 → `session/prompt`；不存在 → spawn → `session/new` → `session/prompt`；`resume_id` → `session/resume` |
| `stop_session` | `session/cancel` 通知 + 超时 kill 兜底 |
| `store_oc_session` | 存 OC session id（`ses_xxx`） |
| `check_oc_session_exists` | `session/list` 或 OC 数据目录检查 |
| `set_oc_settings` / `get_oc_settings` | 读写 opencode.json（委托 oc_config.rs） |
| `get_auto_mode_status` | 读 opencode.json permission 规则 |
| `install_opencode` | 安装 OC CLI（PS1 脚本模式，沿用 cc-gui 安装器思路） |
| `check_skill_installed` | 扫 OC skills 目录 |
| 会话 CRUD / 消息持久化 / 文件 / Git / 翻译 | 从 cc-gui 迁移（引擎无关） |

### 3.5 权限体系设计

OC 模型：`opencode.json` 的 `permission` 规则集 + 每次敏感操作 `session/request_permission` 由客户端决策。
...
| plan | plan agent（禁用编辑类工具） | 按 ACP plan 模式协商，`[待实测]` |
| auto | `"*": "ask"` | 处理器自动回 allow（不弹窗） |

**免审批场景（approved_scenarios）**：迁移 cc-gui 的 SQLite 表；审批处理器先查 `is_approved(tool_name, pattern)`，命中直接回 allow。ACP `PermissionOption[]` 的「记住」选项 → 写入该表。
...
> 实际模型 ID（如是否带 `[1M]` 后缀、OC 内置 deepseek provider 的注册名）以阶段 0 `opencode models` 实测为准，映射表维护在 provider.rs（仅一张表，无 Provider 维度）。
...
```
src-tauri/resources/preset/          # 随应用打包的预置资源（只读，不随 OC 数据变动）
...
- API Key 表单保留（复用 cc-gui 设置面板外观），保存到系统钥匙串 + 同步 opencode.json（由 oc_config.rs 统一管理，预置合并逻辑也在其中）
...
**与引擎配置的关系**：应用配置中含引擎相关项（模型、权限模式、API Key 引用）由 `oc_config.rs` 同步到 OC 的 `opencode.json`；应用 UI 配置（主题/语言等）仅存在于 settings.json。
...
- 采用文件监听（tauri-plugin-fs watch 或轮询），**GUI 表单、JSON 编辑器、agent 修改三条路径统一走文件**，天然一致
- 引擎相关项变更后由 oc_config.rs 增量同步 opencode.json（不重启引擎进程，下次会话生效；模型切换影响新会话）
...
| 步骤 | 内容 | 说明 |
|------|------|------|
| 1 | 初始化 Tauri 2 + Vue 3 + TS + Tailwind 4 + DaisyUI 5 工程 | 与 cc-gui 技术栈一致 |
...
| 操作 | 文件 | 说明 |
|------|------|------|
| 新增 | `src-tauri/src/acp_client.rs` | ACP 客户端 |
| 新增 | `src-tauri/src/oc_protocol.rs` | 事件映射层 |
| 新增 | `src-tauri/src/oc_config.rs` | opencode.json 生成 |
| 新增 | `src-tauri/src/process.rs` | ACP 进程管理（参考 cc-gui 结构新写） |
| 新增 | `src-tauri/src/lib.rs` | 命令集（迁移引擎无关命令 + 新增 OC 命令） |
| 新增 | `src-tauri/src/provider.rs` | DeepSeek 模型映射表（展示名 ↔ 模型 ID） |
| 新增 | `src-tauri/src/session.rs` | 会话管理（迁移 + 字段改名） |
| 新增 | `src-tauri/src/db.rs` | SQLite（迁移 schema） |
| 新增 | `src-tauri/binaries/` | sidecar 二进制（构建脚本下载 + 三元组重命名） |
| 新增 | `scripts/fetch-opencode-bin.ts`（或等） | OC 二进制下载/重命名脚本 |
| 修改 | `src-tauri/tauri.conf.json` | `bundle.externalBin` 声明 sidecar |
| 修改 | `src-tauri/capabilities/default.json` | `shell:allow-spawn` sidecar 权限 |
| 新增 | `src-tauri/resources/preset/` | 预置包（skills/plugins/mcp/instructions/preset.json） |
| 新增 | `src-tauri/src/preset.rs` | 预置初始化逻辑（拷贝 + opencode.json 合并 + 版本管理） |
| 新增 | `src-tauri/src/settings.rs` | 应用配置体系（默认值 + settings.json 读写 + schema 校验 + 文件监听） |
...
| 删除（不迁移） | cc-gui 的 `protocol.rs` CC 解析 | OC 唯一引擎，无需兼容 |
...
| 拷贝+适配 | `src/lib/tauri-bridge.ts` | 命令名调整 |
...
| OC session/update 结构随版本漂移 | 中 | oc_protocol.rs 集中适配 + fixture 单测；sidecar 版本随应用锁定 |
...
| 阶段 | 任务 | 预估工时 |
|------|------|----------|
| 阶段 0 | 实测 `opencode acp`：握手/建会话/prompt/流式/权限请求真实报文，锁定协议细节（附录 B） | 0.5 天 |
| 阶段 1 | 新项目脚手架（Tauri 2 + Vue 3 + 构建链） | 0.5 天 |
| 阶段 2 | 前端设计资产迁移（布局/会话/文件/工具栏/共享 + 状态层 + locales） | 1-1.5 天 |
| 阶段 3 | acp_client.rs + oc_protocol.rs（事件映射 + fixture 单测） | 2 天 |
| 阶段 4 | process.rs + lib.rs + oc_config.rs + provider.rs（引擎集成） | 1.5 天 |
| 阶段 5 | 设置面板适配 + 权限审批联动 + 恢复/分叉 | 1 天 |
| 阶段 6 | **应用配置体系**：settings.rs + schema + JSONC 编辑器 + 文件监听生效 + Agent 改配置验证 | 2 天 |
| 阶段 7 | **sidecar 打包**：下载脚本 + externalBin 配置 + 独立运行验证（无系统 OC 环境） | 1 天 |
| 阶段 8 | **预置配置体系**：预置包制作 + preset.rs 初始化 + 首次引导页 | 1.5 天 |
| 阶段 9 | 全量测试 + 打包（NSIS/MSI）+ 文档 | 1.5 天 |
| **合计（一期）** | | **约 12-13 天** |
...
2. 写一个临时 Node/Python 脚本连接 `opencode acp`（附录 B 步骤）
3. 逐项跑通并记录：initialize → session/new → session/prompt → 流式事件 → 权限请求 → cancel → resume
4. 产出：`docs/实测记录.md`（原始报文 + 附录 C 清单逐项结论）

**阶段 1：脚手架**
1. `npm create tauri-app`（Vue-TS 模板）或复制 cc-gui 的工程配置（package.json/vite/tsconfig/tauri.conf.json 参考）
2. 技术栈锁定：Vue 3 + TS + Pinia + Tailwind 4 + DaisyUI 5 + CodeMirror 6 + marked + mermaid
3. 验证 `npm run dev:tauri` 空壳可跑
...
3. 拷贝 `src/lib/{utils,pinyin,tauri-bridge}.ts`（tauri-bridge 后续适配命令名）
4. 删除 CC 专属前端（引擎选择、CC 安装引导等）
5. 编译通过（vue-tsc）后再进入下一阶段

**阶段 3：协议层**
1. `acp_client.rs`：帧读写（每行 JSON）+ 请求方法 + 通知分发
2. `oc_protocol.rs`：sessionUpdate → StreamFrontendEvent 映射（表见 3.1.2）+ 权限请求解析
3. fixture 单测：用阶段 0 的真实报文做输入样例（仿 cc-gui protocol.rs 测试风格）
4. `cargo test` 通过

**阶段 4：引擎集成**
1. `process.rs`：spawn ACP（sidecar 优先/系统 OC 兜底）、进程注册、退出处理
2. `lib.rs`：send_message / stop_session / store_oc_session / check_oc_session_exists
3. `oc_config.rs`：opencode.json 生成（DeepSeek apiKey 引用 + 权限规则 + 预置 merge）
4. `provider.rs`：DeepSeek 模型映射表
5. 端到端冒烟：GUI 发消息 → 流式回复

**阶段 5：交互完善**
1. 设置面板 API Key/模型区适配
2. 权限审批：request_permission → control_request 事件 → 弹窗 → 响应；approved_scenarios 联动
3. 恢复（session/resume）与分叉（fork 能力协商）
4. 异常路径：进程崩溃/超时/审批取消

**阶段 6：应用配置体系**
1. `settings.rs`：默认值 + settings.json 读写 + JSONC 解析 + schema 校验
2. `settings.schema.json`：全字段定义（表单与 agent 共用）
3. 文件监听（tauri-plugin-fs watch 或轮询）+ config-changed 事件
4. `SettingsJsonEditor.vue`：CodeMirror + schema 补全
5. 验证 agent 改配置闭环（临时会话里让引擎改 settings.json 看生效）

**阶段 7：sidecar**
1. `scripts/fetch-opencode-bin.ts`：下载 zip → 解压 → 加三元组后缀 → `src-tauri/binaries/`
2. `tauri.conf.json` externalBin + `capabilities/default.json` 权限
3. 无系统 OC 环境验证（临时改名系统 opencode 或 PATH 隔离）

**阶段 8：预置体系**
1. 制作 `src-tauri/resources/preset/`（skills/plugins/mcp/instructions/preset.json，内容待业务定）
2. `preset.rs`：首次启动拷贝 + opencode.json merge + 预置版本管理
3. 引导页 `onboarding/`：2 步（API Key → 测试连接）

**阶段 9：收尾**
1. cargo test + vitest + e2e（Playwright + 真实 ACP fixture）
2. `tauri build --bundles nsis` 打包验证
...
- `tauri.conf.json` → `bundle.externalBin: ["binaries/opencode"]`；二进制命名带目标三元组后缀（Windows：`opencode-x86_64-pc-windows-msvc.exe`）
- 启动：Rust `app.shell().sidecar("binaries/opencode")` 或前端 `Command.sidecar`；stdin/stdout 管道与普通 Command 一致
- 权限：`capabilities/default.json` 加 `shell:allow-spawn`（或 `shell:allow-execute`）+ `sidecar: true`
...
- 下载后解压即得 opencode 可执行文件，重命名加三元组后缀放入 `src-tauri/binaries/`
...
**目的**：拿到 `opencode acp` 的真实报文，逐项验证附录 C 清单，为 acp_client.rs / oc_protocol.rs 提供 fixture。

**步骤**：

1. 确认 OC 版本：`opencode --version`（记录版本号，协议结论绑定该版本）
2. 准备临时目录：`C:\Users\MaxNull\AppData\Local\Temp\opencode\acp-probe`（已存在，可直接使用）
3. 写 Node 脚本（或用 `opencode acp` 的官方 SDK 思路）：
   - spawn `opencode acp --cwd <临时目录>`
   - 按行写 stdin / 按行读 stdout
   - 依次发送并记录：
     a. `{"jsonrpc":"2.0","id":1,"method":"initialize","params":{...}}` → 记录响应（capabilities：resume/fork/mcp 支持情况）
     b. `session/new`（cwd）→ 记录 sessionId 格式
     c. `session/prompt`（"用一句话介绍你自己"）→ 记录完整事件序列（重点：agent_message_chunk 的 messageId/part 结构）
     d. 触发工具调用（"读取当前目录的文件列表"）→ 记录 tool_call / tool_call_update 结构 + 若触发权限则记录 request_permission 完整 JSON（PermissionOption 字段）
     e. `session/cancel` → 记录中断行为
     f. 再次 prompt 后 `session/resume` → 记录重放行为
4. 全部原始报文存入 `docs/实测记录.md`，并标注每条对应的协议结论
...
| C-6 | OC 内置 DeepSeek 是否需要显式 provider 块 | 实测仅 apiKey 注入能否连接 | oc_config.rs 简化度 | 待实测 |
...
阶段 0 实测产物直接建到 `src-tauri/tests/fixtures/`（acp_client / oc_protocol 的测试种子）：
