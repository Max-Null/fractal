...
**职责**：生成/合并 **分形独立配置目录**（D17：`XDG_CONFIG_HOME` 指向的分形数据目录，如 `%APPDATA%/oc-gui/oc-data/config/opencode/`）下的 `opencode.json`，并将应用配置中的引擎相关项（DeepSeek API Key 引用、模型、权限模式）同步到 OC 配置。
...
env:   XDG_CONFIG_HOME=<分形数据目录>/config（D17 隔离，实测定案）
```
...
sequenceDiagram
    ...
    OC->>Main: /event SSE: 权限请求事件（tool, input, permissionID）
    Main->>Main: 查会话级免审批（approved_scenarios）/ 持久化 permission 规则命中？
    alt 命中 allow 规则
        Main-->>OC: POST /session/:id/permissions/:permissionID {response: allow}
    else 未命中
        Main-->>前端: stream-event(control_request)
        前端-->>用户: 弹窗展示工具与输入
        用户-->>前端: 选择 允许/会话内允许/拒绝
        前端->>Main: 审批决策
        alt 选择「记住此场景」（持久化）
            Main->>Main: 写回 opencode.json permission 规则（下次会话生效，P9 拍板）
        end
        Main-->>OC: POST /session/:id/permissions/:permissionID {response, remember}
    end
```
